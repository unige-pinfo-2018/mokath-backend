...
if [[ "$(docker ps -a -q -f "name=$CONT_NAME" 2> /dev/null)" != "" ]]; then
	...
	docker run -d --ip="172.18.0.5" --net="$NETWORK_NAME" -v es-data:/usr/share/elasticsearch/data -v $(pwd)/elasticsearch.yml:/usr/share/elasticsearch/config/elasticsearch.yml -p 9201:9200 -p 9301:9300 -e "discovery.type=single-node" --name $CONT_NAME $IMG_NAME
fi
